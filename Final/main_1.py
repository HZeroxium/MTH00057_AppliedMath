...
def run_models_with_single_feature(
    ...
    # Tìm ra mô hình tốt nhất với feature tốt nhất
    best_feature = min(best_results, key=lambda x: best_results[x][1])
    best_model, best_mse = best_results[best_feature]
    print(
        f"\nBest feature: {best_feature} | Best Model: {best_model} | MSE: {best_mse:.4f}"
    )
...
def main():
    ...
    run_models_with_different_splits(
        ...
        print_sub_table=print_sub_table,
    )
